...
#  2) Prompt for:
#      a) existing username to run rootless cloudflared (fail if missing)
#      b) cloudflared image tag
#      c) cloudflared tunnel token (dashboard-generated)
#      d) next-hop gateway IP for RFC1918 routes via eth1   <-- UPDATED
...
#      - RFC1918 prefixes routed via *prompted* eth1 next-hop gateway
#      - ensure eth1 does NOT have a default route
#  7) Pull cloudflared image from docker.io using fully-qualified name + tag
#  8) Create Quadlet drop-in(s):
#      - [Container] Image=... Pull=never
#      - [Container] Exec=... --no-autoupdate ... --token ...
#      - [Service] LimitNOFILE=250000
#  9) Create Quadlet base file in:
#      ~cloudflare_user/.config/containers/systemd/cloudflared.container
#
# Notes:
#  - Persistence of routes uses NetworkManager ONLY if it's active; otherwise
#    routes are applied at runtime only.
#  - The tunnel token is stored on disk in a drop-in file (0600). Protect it.
#
# Usage:
#   sudo bash cloudflared-two-nic-setup.sh
#------------------------------------------------------------------------------
...
user_systemctl() {
  ...
  # Ensure a runtime dir exists for the user instance
  # (on linger-enabled systems, systemd will normally create it, but not always immediately)
  sudo mkdir -p "/run/user/${uid}"
  sudo chown "${uid}:${uid}" "/run/user/${uid}"
  sudo chmod 0700 "/run/user/${uid}"

  # Run systemctl --user with the correct runtime dir
...
# Get default gateway for a given interface from "ip route show"
...
is_ipv4() {
  local ip="$1"
  # Basic IPv4 dotted quad check (sufficient for a prompt)
...
configure_routes_two_nic() {
  ...
  # Add/replace RFC1918 routes via *prompted* eth1 gateway at runtime
  ...
  if is_systemctl_active NetworkManager.service && command -v nmcli >/dev/null 2>&1; then
    ...
    # Avoid duplicates by removing any existing matching routes first (best-effort)
...
create_quadlet_rootless() {
  ...
  # Base quadlet (no secrets). Token Exec is provided by drop-in.
...
EOF
  ...
  info "Reloading generator and starting service as user $u (with XDG_RUNTIME_DIR)"
...
main() {
  ...
  # Validate NIC presence early
  iface_exists eth0 || die "Interface eth0 not found. This script expects eth0 as the primary NIC."
  iface_exists eth1 || die "Interface eth1 not found. Add/attach the second NIC and ensure it appears as eth1, then re-run."

  install_packages

  echo
  read -r -p "Enter existing username to run cloudflared (rootless): " CF_USER
  [[ -n "${CF_USER}" ]] || die "Username cannot be empty"
  user_exists "${CF_USER}" || die "User does not exist: ${CF_USER}"
  ...
  echo "  sudo -u ${CF_USER} systemctl --user status cloudflared.service -l --no-pager"
